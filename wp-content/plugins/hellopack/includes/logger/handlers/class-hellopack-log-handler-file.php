...
class HPack_Log_Handler_File extends HPack_Log_Handler
{
    /**
    ...
    public function __construct($log_size_limit = null)
    {
        ...
        $this->log_size_limit = apply_filters('hellopack_log_file_size_limit', $log_size_limit);
    ...
    public function remove($handle)
    {
        ...
        if (isset($logs[$handle]) && $logs[$handle]) {
            $file = realpath(trailingslashit(HP_UPDATER_LOG_DIR) . $logs[$handle]);
            if (0 === stripos($file, realpath(trailingslashit(HP_UPDATER_LOG_DIR))) && is_file($file) && is_writable($file)) { // phpcs:ignore WordPress.VIP.FileSystemWritesDisallow.file_ops_is_writable
    ...
    public static function get_log_file_path($handle)
    {
        if (function_exists('wp_hash')) {
            return trailingslashit(HP_UPDATER_LOG_DIR) . self::get_log_file_name($handle);
        } else {
            hp_doing_it_wrong(__METHOD__, __('This method should not be called before plugins_loaded.', 'hellopack'), '2.1.0');
            return false;
        }
    }
    ...
    public static function delete_logs_before_timestamp($timestamp = 0)
    {
        ...
        foreach ($log_files as $log_file) {
            $last_modified = filemtime(trailingslashit(HP_UPDATER_LOG_DIR) . $log_file);

            if ($last_modified < $timestamp) {
                @unlink(trailingslashit(HP_UPDATER_LOG_DIR) . $log_file); // @codingStandardsIgnoreLine.
            }
        }
    }
    ...
    {
        $files = @scandir(HP_UPDATER_LOG_DIR); // @codingStandardsIgnoreLine.
        $result = array();
